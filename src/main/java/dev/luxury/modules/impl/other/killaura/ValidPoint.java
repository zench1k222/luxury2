...
import dev.luxury.modules.impl.other.killaura.rotate.DeltaRotate;
import dev.luxury.modules.impl.other.killaura.rotate.RotateUtils;
...
public class ValidPoint {
    ...
    private boolean isValidPoint(Vec3d startPoint,
        ...
        var hit = RayTrace.raycast(startPoint, endPoint, RaycastContext.ShapeType.COLLIDER);
    ...
    private double calculateRotationDifference(Vec3d startPoint, Vec3d endPoint, Rotate initialRotate) {
        if (initialRotate == null) return Double.POSITIVE_INFINITY;
        var targetRotation = RotateUtils.fromVec3d(endPoint.subtract(startPoint));
        DeltaRotate delta = initialRotate.rotationDeltaTo(targetRotation);
