...
public class ModeSettingRenderer extends SettingRenderer {
    ...
    public float render(DrawContext context, Setting setting, float x, float y, float width, int mouseX, int mouseY, float scrollOffset) {
        ...
        for (String mode : modeSetting.getModes()) {
            ...
            int bgColor = isSelected ? 0xFF00ff11 : (isHovered ? 0xFF333333 : 0xFF252525);
            RenderUtil.drawRoundedRect(context.getMatrices(), x + 10, actualY, width - 10, MODE_ITEM_HEIGHT, new Vector4f(3f, 3f, 3f, 3f), bgColor);

            float radioX = x + 15;
            float radioY = actualY + 4;
            float radioSize = 10f;

            RenderUtil.drawRoundedRect(context.getMatrices(), radioX, radioY, radioSize, radioSize, new Vector4f(5f, 5f, 5f, 5f), isSelected ? 0xFF00ff11 : 0xFF808080);

            if (isSelected) {
                RenderUtil.drawRoundedRect(context.getMatrices(), radioX + 2, radioY + 2, radioSize - 4, radioSize - 4, new Vector4f(5f, 5f, 5f, 5f), Color.WHITE.getRGB());
            }

            FontDraw montserratMaloy = FontHelper.monsterrat[14];
            montserratMaloy.drawFontLeft(context.getMatrices(), mode, x + 30, actualY + 5, isSelected ? Color.WHITE.getRGB() : 0xFFAAAAAA);
