...
public class SliderSettingRenderer extends SettingRenderer {
    ...
    public float render(DrawContext context, Setting setting, float x, float y, float width, int mouseX, int mouseY, float scrollOffset) {
        ...
        RenderUtil.drawRoundedRect(context.getMatrices(), sliderX, sliderY, sliderWidth, sliderHeight, new Vector4f(2f, 2f, 2f, 2f), 0xFF404040);

        float fillPercent = (float) ((value - min) / (max - min));
        float fillWidth = sliderWidth * fillPercent;
        RenderUtil.drawRoundedRect(context.getMatrices(), sliderX, sliderY, fillWidth, sliderHeight, new Vector4f(2f, 2f, 2f, 2f), 0xFF00ff11);

        float indicatorX = sliderX + fillWidth - 4;
        float indicatorY = sliderY - 2;
        float indicatorSize = 8f;
        RenderUtil.drawRoundedRect(context.getMatrices(), indicatorX, indicatorY, indicatorSize, indicatorSize, new Vector4f(4f, 4f, 4f, 4f), Color.WHITE.getRGB());
